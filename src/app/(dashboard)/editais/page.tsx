  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-slate-800">Central de Editais</h1>
        ...
        {(profile?.role === 'teacher' || profile?.role === 'admin') && (
          <Link
            href="/editais/novo"
            className="flex items-center gap-2 bg-blue-700 hover:bg-blue-800 text-white text-sm font-medium px-3 py-2 rounded-lg transition-colors flex-shrink-0"
          >
            <PlusCircle size={15} />
        ...
        <>
          ...
          <div className="md:hidden space-y-3">
            {edicts.map((edict: any) => (
              <div key={edict.id} className="bg-white rounded-xl border border-slate-200 p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <Link
                    href={`/editais/${edict.id}`}
                    className="text-blue-600 hover:text-blue-800 font-medium text-sm leading-snug flex-1"
                  >
                    {edict.title}
                  ...
                  {edict.status && (
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0 ${statusStyle[edict.status]}`}>
                ...
                <div className="flex flex-wrap gap-2 items-center text-xs text-slate-400">
                  {edict.category && (
                    <span className={`px-2 py-0.5 rounded-full font-medium ${categoryColors[edict.category] ?? 'bg-slate-100 text-slate-600'}`}>
                      {edict.category}
                    </span>
                  )}
                  {edict.published_at && (
                    <span>
                      Edital: {new Date(edict.published_at).toLocaleDateString('pt-BR')}
                    </span>
                  )}
                  <span>
                    Atualizado: {new Date(edict.last_update).toLocaleDateString('pt-BR')}
                  </span>
                </div>
              </div>
            ))}
                ...
                {edicts.map((edict: any) => (
                  <tr key={edict.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-5 py-4 text-slate-500 whitespace-nowrap">
                      {new Date(edict.last_update).toLocaleDateString('pt-BR')}
                      <br />
