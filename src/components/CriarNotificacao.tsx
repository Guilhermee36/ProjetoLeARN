...
export default function CriarNotificacao({ authorName }: { authorName: string }) {
  ...
  async function handleSend(e: React.FormEvent) {
    ...
    // Busca TODOS os perfis (alunos + professores)
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 space-y-4">
        ...
        {sent ? (
          <div className="text-center py-6 space-y-2">
            ...
            <p className="font-medium text-green-700">Notificação enviada para todos!</p>
          </div>
        ) : (
          <form onSubmit={handleSend} className="space-y-4">
            <div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-500">
              Enviando como <span className="font-medium text-slate-700">{authorName}</span>
            </div>
            ...
            {referenceType !== 'general' && (
              <>
                ...
                <div className="space-y-2">
                  <Label>ID do {referenceType === 'edict' ? 'edital' : 'projeto'} (para criar link)</Label>
